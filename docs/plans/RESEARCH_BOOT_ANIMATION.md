...
- [ ] dotLottie / Rive 运行时的准确体积未获得可靠数据（公开来源不一致）
- [ ] Playwright 截图在本环境能否跑通未验证（长命令被判定为 watch 服务并超时）
- [ ] 和风天气图标库（qwd/Icons）的许可未核实
- [ ] 未实测中低端安卓上的真实帧率（现有实现的掉帧程度未量化）
- [ ] 清理：调研过程中生成的临时文件 `vitest-last.log` 需手动删除
