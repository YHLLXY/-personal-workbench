...
- [ ] `npm test` 全绿（含 repeat / 排序 / 契约新增段）
- [ ] `npm run build` + `check:bundle`：预期总量 ≤ 1095/1120（增量估 +10KB 内，今日页为懒 chunk）、max chunk ≤ 340
- [ ] `npm run lint` ≤ 12（对比改动前后）
- [ ] `npx playwright test` 全绿（含新用例）
- [ ] 用户在场执行 011 → 云端回归：建重复任务（完成滚动/撤销/跳过）、清单增删勾选、拖拽（桌面把手 + 移动长按无放大镜）、老数据顺序与升级前一致
