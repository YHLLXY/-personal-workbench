...
> 2026-08-30 调研整理。状态：**待拍板**（见 §7 三个问题）。
...
1. **城市坐标**：固定用哪个城市？（告诉我城市名即可，我用 Open-Meteo Geocoding 查坐标写进配置；不选则默认郑州/或你常驻城市）
2. **播放频率**：推荐「每天首次打开播放、当天后续跳过」；备选「每次会话都播」（sessionStorage，关标签页即重置）——频繁使用建议前者
3. **夜晚流星彩蛋**：夜晚场景偶发流星（纯加分项，零成本），要不要？
