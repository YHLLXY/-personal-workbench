...
const MOBILE_TABS = [
  ...modules.map(m => ({ moduleId: m.id, sub: m.children[0] })),
  { moduleId: '__me__', sub: { id: 'me', name: '我的', icon: Settings, path: '/settings', component: () => null } },
]
    ...
    <div className="flex min-h-dvh bg-background">
      ...
      <aside className="hidden md:flex md:flex-col md:w-[200px] xl:w-[224px] shrink-0 border-r border-border bg-card/60 backdrop-blur px-2 py-4">
        ...
        <div className="border-t border-border pt-2">
          <button onClick={() => navigate('/settings')} className="flex w-full items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] text-muted-foreground hover:bg-muted">
      ...
      {/* 移动端底部 Tab */}
      <nav className="md:hidden fixed bottom-0 inset-x-0 z-30 bg-card/95 backdrop-blur border-t border-border grid grid-cols-5 pb-[env(safe-area-inset-bottom)]">
        {MOBILE_TABS.map(t => (
