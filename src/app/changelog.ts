...
export const CHANGELOG: ChangelogEntry[] = [
  {
    version: 'v1.25.0',
    date: '2026-09-13',
    title: '今日待办卡片信息升级：完整标题 · 任务备注 · 逾期日期',
    items: [
      '新增：任务备注——弹窗里可以给任务写补充说明，直接显示在卡片标题下方（灰色小字，最多两行，全文悬停或进编辑可见）',
      '升级：任务标题不再截断——长任务完整换行显示，一眼看全你写的内容',
      '升级：卡片信息更清爽——「今日焦点」「进行中」「清单进度」挪到信息行，标题行只留优先级圆点+文字',
      '新增：逾期任务卡片补上源到期日（红色「M月D日 周X」），不用再猜它逾期多久了',
      '内部：新增迁移 012（任务备注列），需在设置页看到本版本前由用户在 Supabase 执行',
    ],
  },
  {
    version: 'v1.24.0',
    date: '2026-09-11',
    title: '任务系统升级：重复任务 · 子清单 · 拖拽排序 · 改期菜单',
    items: [
      '新增：重复任务——每天 / 每个工作日 / 每周（可多选周几）/ 每月 / 自定义间隔，可选按计划日或按完成日推进；勾完自动滚到下一次，行内「每天」徽章，撤回到今日，菜单可「跳过一次」（逾期未做按 Apple 提醒事项式跳过不补）',
      '新增：任务子清单——行内展开勾选 / 增删条目，标题行尾显示 x/y 进度（全部勾完不自动完成父任务，与滴答清单一致）',
      '新增：今日待办手动拖拽排序——把手拖拽（桌面悬停显形、移动端常显），换位走滑移动画；排序从此完全归你，优先级圆点降为视觉标记',
      '新增：「进行中」状态——弹窗三态（待办 / 进行中 / 将来）+ 行内蓝点徽章，今日区排在待办之前',
      '新增：任务改期菜单——今天 / 明天 / 后天 / 下周一 / 自选日期；逾期区加「全部顺延到今天」一键清理',
      '新增：快速捕获「存入将来收件箱」——先扔进将来区，之后再决定什么时候做；将来区条目可一键移到今天或指定日期',
      '内部：排序语义改为「优先级烘焙进排序值」（迁移 011 回填存量数据，升级前后默认顺序完全一致）；拖拽用半序中点算法，只写被拖动的那一条记录',
    ],
  },
  {
    version: 'v1.23.2',
