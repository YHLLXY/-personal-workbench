...
export function FolderTree({ folders, selectedId, onSelect }: Props) {
  ...
  function rename(f: Folder) {
    ...
    update.mutate({ id: f.id, patch: { name: name.trim() } }, { onSuccess: () => toast.success('已重命名') })
  }
          ...
          <div key={f.id}>
            ...
            >
              ...
              <span className="flex shrink-0 items-center gap-0.5 text-muted-foreground/60 md:opacity-0 md:group-hover:opacity-100">
                <button onClick={e => { e.stopPropagation(); newFolder(f.id) }} aria-label="新建子文件夹" className="hover:text-foreground"><FolderPlus className="size-3.5" /></button>
                <button onClick={e => { e.stopPropagation(); rename(f) }} aria-label="重命名" className="hover:text-foreground"><Pencil className="size-3.5" /></button>
  ...
  return (
    <div className="space-y-0.5">
      ...
      {renderTree(null, 0)}
      <Button variant="ghost" size="sm" className="mt-1 w-full justify-start text-muted-foreground" onClick={() => newFolder(null)}>
