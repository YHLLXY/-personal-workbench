...
import { formatSeconds, breakForFocusIndex, FOCUS_MIN, SHORT_BREAK_MIN, LONG_BREAK_MIN, type Phase } from '@/lib/pomodoro'
...
export default function Pomodoro() {
  ...
  useEffect(() => {
    ...
    if (phase === 'focus') {
      mutate({ minutes: FOCUS_MIN })
      const nextPhase = breakForFocusIndex(focusIndex)
      setPhase(nextPhase)
      setRemaining(nextPhase === 'long' ? LONG_BREAK_MIN * 60 : SHORT_BREAK_MIN * 60)
      setFocusIndex(i => i + 1)
    } else {
      setPhase('focus')
    ...
    setRunning(false)
  }, [remaining, phase, focusIndex, mutate])
  ...
  const pct = phase === 'focus' ? remaining / (FOCUS_MIN * 60) : remaining / (phase === 'long' ? LONG_BREAK_MIN * 60 : SHORT_BREAK_MIN * 60)
    ...
    <div className="mx-auto max-w-md flex flex-col items-center gap-6 pt-6">
      ...
      <div className="flex gap-1 bg-muted rounded-full p-1">
        {(['focus', 'short', 'long'] as const).map(p => (
          <button key={p} onClick={() => { setPhase(p); setRemaining(p === 'focus' ? FOCUS_MIN * 60 : p === 'long' ? LONG_BREAK_MIN * 60 : SHORT_BREAK_MIN * 60); setRunning(false) }}
      ...
      <div className="flex gap-3">
        ...
        <Button size="lg" variant="outline" onClick={() => { setRunning(false); setRemaining(FOCUS_MIN * 60); setPhase('focus') }}><RotateCcw className="size-4 mr-1.5" />重置</Button>
      </div>
