  ...
  return (
    <div>
      <div className="flex gap-2 mb-3">
        <Input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && !create.isPending && name.trim() && create.mutate({ name: name.trim() }, { onSuccess: () => { setName(''); toast.success('习惯已创建') } })} placeholder="新习惯，如：阅读 30 分钟" className="h-9 text-sm" />
        <Button size="sm" disabled={create.isPending} onClick={() => { if (name.trim()) { create.mutate({ name: name.trim() }, { onSuccess: () => { setName(''); toast.success('习惯已创建') } }) } }}>
          <Plus className="size-3.5 mr-1" />创建
        </Button>
      </div>
      <div className="flex flex-wrap gap-1.5 mb-4">
        {PRESETS.map(p => <button key={p} disabled={create.isPending} onClick={() => create.mutate({ name: p }, { onSuccess: () => toast.success(`已添加「${p}」`) })} className="text-[11px] border border-border rounded-full px-2.5 py-1 text-muted-foreground hover:border-primary hover:text-primary disabled:opacity-50">{p} +</button>)}
      </div>
